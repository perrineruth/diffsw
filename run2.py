...
def train_model(seq_array, verbose=False):
    ...
    model_aln = jax.jit(model_trainMRF._get_model(initialize_params=False, return_aln=True),device=cpus[0])
    idx = np.array(range(len(x)))
    inputs = {"x":self.X[idx], "lengths":self.lengths[idx],
                  "x_ref":self.X_ref, "key":[None]*3}
    aln, sm = model_aln(model_trainMRF.opt.get_params(), inputs)
#     aln = (model_trainMRF.get_aln(np.array(range(len(x))))[0])
    contacts = model_trainMRF.get_contacts()
    
    return ms, aln, contacts, lens
...
def run_data():
    ...
    for i, (protein, seq_array_dict) in enumerate(dict_MSA.items()):
        ...
        dict_MSA[protein]['ms'], dict_MSA[protein]['alignments'], dict_MSA[protein]['contacts'],dict_MSA[protein]['lens'] = train_model(seq_array)
